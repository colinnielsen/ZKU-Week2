...
describe('Custom Tests', function () {
  ...
  it('[assignment] ii. deposit 0.1 ETH in L1 -> withdraw 0.08 ETH in L2 -> assert balances', async () => {
    ...
    const aliceBalanceBeforeWithdraw = await token.balanceOf(alice.address)
    ...
    expect(recipientBalance).to.be.equal(aliceBalanceBeforeWithdraw.add(aliceWithdrawAmount))
  ...
  it('[assignment] iii. see assignment doc for details', async () => {
    ...
    const bobShieldedAddress = bobKeypair.address()

    //  -> Alice deposits 0.08 Edit: 0.13 ETH in L1
    ...
    const poolBalance = await token.balanceOf(tornadoPool.address)
    expect(poolBalance).to.be.equal(aliceDepositAmount)

    //  -> Alice sends 0.06 ETH to Bob in L2
    const bobSendAmount = utils.parseEther('0.06')
    const bobSendUtxo = new Utxo({ amount: bobSendAmount, keypair: Keypair.fromString(bobShieldedAddress) })
    const aliceChangeUtxo = new Utxo({
    ...
    const bobChangeUtxo = new Utxo({ amount: constants.Zero, keypair: bobKeypair })
    await transaction({
      tornadoPool,
    ...
    const aliceWithdrawUTXO = new Utxo({ amount: constants.Zero, keypair: aliceKeypair })
    await transaction({
      tornadoPool,
    ...
    const bobBalance_final = await token.balanceOf(bobEthAddress)
    expect(bobBalance_final).eqls(bobSendAmount)

    const aliceBalance_final = await token.balanceOf(aliceAddress)
    expect(aliceBalance_final).eqls(parseEther("10000").sub(aliceDepositAmount))

    const poolBalance_final = await token.balanceOf(tornadoPool.address)
    expect(poolBalance_final).eqls(constants.Zero)
